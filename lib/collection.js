...
var modifyjs = require("../mingo-modify"); // require('modifyjs');
var aggregate = require("../mingo-aggregate");
...
module.exports = function Collection(db, state) {
  ...
  debug(
    "initializing instance of `%s` with %s documents",
    name,
    state.documents ? state.documents.length : undefined
  );
  ...
  var iface = {
    ...
    get namespace() {
      return db.databaseName + "." + name;
    },
    get writeConcern() {
      NotImplemented();
    },

    // TODO: complete full functions
    aggregate: function (pipelines) {
      return {
        toArray() {
          // @ts-ignore
          return aggregate(db, iface, pipelines);
        },
      };
    },
      ...
      asyncish(function () {
        cursor(state.documents || [], opts).toArray((err, docsToRemove) => {
          ...
          if (docsToRemove.length) {
            ...
            _.remove(state.documents || [], (document) =>
              _.includes(idsToRemove, document._id)
            );
            debug(state.documents.length);
          ...
          callback(null, {
            result: { n: docsToRemove.length, ok: 1 },
            deletedCount: docsToRemove.length,
            connection: db,
          });
        });
      });
    ...
    deleteOne: function (filter, options, callback) {
      ...
      asyncish(function () {
        ...
        var docs =
          deletionIndex === -1 ? [] : state.documents.splice(deletionIndex, 1);
    ...
    findOne: function () {
      ...
      if (!opts.callback) return crsr.next();
    ...
    findOneAndUpdate: function (selector, data, options, callback) {
      ...
      this.updateOne(selector, data, options)
        .then(function (opResult) {
          ...
          if (options.upsert === true && opResult.upsertedId._id)
            findQuery._id = opResult.upsertedId._id;
          self
            .findOne(findQuery)
            .catch(callback)
            .then(function (doc) {
              ...
              if (options.upsert) {
                ...
                if (!!opResult.upsertedCount)
                  findResult.lastErrorObject.upserted =
                    opResult.upsertedId._id.str;
              }
...
function restoreObjectIDs(originalValue, updatedValue) {
  return updatedValue &&
    updatedValue.constructor.name === "ObjectID" &&
    updatedValue.id
    ? ObjectId(updatedValue.id)
    : undefined;
}
...
function operatorArrayToNormalizedObject(array, result) {
  ...
  for (const item of array) {
    ...
    for (const itemKey of Object.keys(item)) {
      assert(
        !Boolean(result[itemKey]),
        `MongoError: cannot infer query fields to set, path '${itemKey}' is matched twice.`
      );
    }
