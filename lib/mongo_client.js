...
MongoClient.connect = function (url, options, callback) {
  ...
  var server =
    servers[url.host] ||
    (servers[url.host] = { databases: {}, persist: MongoClient._persist });
...
function persist() {
  ...
  var inspect =
    (Symbol && Symbol.for("nodejs.util.inspect.custom")) || "inspect";

  ObjectId.prototype.toJSON =
    ObjectId.prototype[inspect] || ObjectId.prototype.inspect;
  out += JSON.stringify(servers, null, 2).replace(
    /"ObjectID\(([0-9a-f]{24})\)"/g,
    'ObjectID("$1")'
  );
...
MongoClient.load = function (filename, callback) {
  ...
  function create_server(server_name) {
    ...
    function create_database(database_name) {
      if (!database_name) return create_server(servers_names.pop());

      var collections = data[server_name].databases[database_name].collections;
      MongoClient.connect(
        "mongodb://" + server_name + "/" + database_name,
        function (err, db) {
          if (err) throw err;
            ...
            db.createCollection(collection.name, function (err, instance) {
              ...
              function insert(doc) {
                ...
                if (
                  ...
                  doc._id.startsWith("ObjectId")
                )
                  doc._id = ObjectId(doc._id);
                instance.update(doc, doc, { upsert: true }, function (
                  err,
                  result
                ) {
                  if (err) throw err;
